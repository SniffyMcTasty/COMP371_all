...
bool RectangleGeometry::hit(const Ray &r, double t_min, double t_max, hit_record &rec) const
{
    ...
    point3 P = r.at(t);
    float outsideP1P2 = (this->p2 - this->p1).cross(P - this->p1).dot(normal);
    if (outsideP1P2 < 0) return false;

    float outsideP2P3 = (this->p3 - this->p2).cross(P - this->p2).dot(normal);
    if (outsideP2P3 < 0) return false;

    float outsideP3P1 = (this->p1 - this->p3).cross(P - this->p3).dot(normal);
    if (outsideP3P1 < 0) {
        float outsideP3P4 = (this->p4 - this->p3).cross(P - this->p3).dot(normal);
        if (outsideP3P4 < 0) return false;

        float outsideP4P1 = (this->p1 - this->p4).cross(P - this->p4).dot(normal);
        if (outsideP4P1 < 0) return false;
    }
